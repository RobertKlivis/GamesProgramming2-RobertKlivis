...
Audio::Audio()
{
	...
	if (device == NULL) //if no device detected
	{
		std::cout << "cannot open sound card" << std::endl; //display the message
	}

	context = alcCreateContext(device, NULL);

	if (context == NULL) //if no context detected
	{
		std::cout << "cannot open context" << std::endl; //display the message
	}
...
unsigned int Audio::loadSound(const char* filename)
{
	...
	unsigned int sourceID, bufferID; //unsigned integers for sourceID and bufferID
	char* soundData = NULL;
	for (unsigned int i = 0; i < datas.size(); i++)
	{
